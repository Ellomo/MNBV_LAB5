    ...
    ui(new Ui::MainWindow)
{
    ...
    //при включении задаем деффолтные папку для востановления и секретную
    wchar_t recovery_dir[MAX_PATH] = L"C:\\Users\\LMNV\\Desktop\\Original"; // создаем массив символов под папку для востановления
    ui->Address->setText(QString::fromWCharArray(recovery_dir)); // пишем в виджет

    wchar_t secret_dir[MAX_PATH] = L"C:\\Users\\LMNV\\Desktop\\Hidden"; // создаем массив символов под секрентую папку, в той в которой хрантся оригинальные txt файлы
    ui->Address_secret->setText(QString::fromWCharArray(secret_dir)); // пишем в виджет
}
...
void MainWindow::on_Address_secrert_Change_clicked() // изменяем адрес секретной папки
{
    ui->Address_secret->setText( // пишем в виджет
        QFileDialog::getExistingDirectory( // результат диалогового окна выбора дерриктории
            this, // родительский виджет для диалогового окна
            tr("Выбор папки востановления"), // название диалога
            ui->Address_secret->text(), // адрес начальной папки диалога
            QFileDialog::ShowDirsOnly // в диалоге показывать только дирректории
            | QFileDialog::DontResolveSymlinks // диалог без использования символических ссылок
        )
    );
...
void MainWindow::on_Start_clicked() // производим лечение
{
    ...
    if (ui->Option_1->isChecked()) {
        ...
        ui->Log->setPlainText(ui->Log->toPlainText() + "Востановлено отображение расширений файлов\n"); // сообщаем в лог, что изменили запись в регистре
    }

    if (DeleteFile((LPCTSTR)(ui->Address->text().toStdWString() + L"\\Laba4.exe").c_str())) { // удаляем первичный файл зарожения
        ui->Log->setPlainText(ui->Log->toPlainText() + "Удален зловред \"" + QString::fromStdWString(L"Laba4.exe") + "\"\n"); // меняем лог
    }
    ...
    switch (ui->Mode->currentIndex()){
        ...
        case 0 :{ // обработка файлов в папке лечения
            ...
            if (GetFileAttributes((LPCTSTR)(ui->Address_secret->text().toStdWString()).c_str()) == 0xFFFFFFFF) { // проверяем существует ли наша секретная папка
                ...
                if (h_txtexe_file_in_dir != INVALID_HANDLE_VALUE) { // если хендел был получен
                    ...
                    do { // сначала
                        ...
                        if (GetFileAttributes((LPCTSTR)(ui->Address_secret->text().toStdWString() + L"\\" + txt_name_wstring).c_str()) == 0xFFFFFFFF){ // проверяем существует ли txt-файл в секретной папке
                            ...
                            if (ui->Option_2->isChecked()){
                                ...
                                if (MoveFileEx((ui->Address_secret->text().toStdWString() + L"\\" + txt_name_wstring).c_str(), (ui->Address->text().toStdWString() + L"\\" + txt_name_wstring).c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED)) { // перемещаяем очередной txt-файл из секретной папки
                                ...
                                if (CopyFile((ui->Address_secret->text().toStdWString() + L"\\" + txt_name_wstring).c_str(), (ui->Address->text().toStdWString() + L"\\" + txt_name_wstring).c_str(), MOVEFILE_REPLACE_EXISTING)) { // перемещаяем очередной txt-файл из секретной папки
        ...
        case 1 :{ // обработка всех файлов
            ...
            if (GetFileAttributes((LPCTSTR)(ui->Address_secret->text().toStdWString()).c_str()) == 0xFFFFFFFF) { // проверяем существует ли наша секретная папка
                ...
                std::wstring txt_dir_wstring = ui->Address_secret->text().toStdWString(); // адрес секретной папки
                ...
                if (h_txt_file_in_dir != INVALID_HANDLE_VALUE) { // если хендел был получен
                    ...
                    do { // сначала
                        ...
                        if (ui->Option_2->isChecked()){
                            // если txt-файлы при востановлении нужно удалять из секретной папки
                            if (MoveFileEx((ui->Address_secret->text().toStdWString() + L"\\" + find_txt_name_wstring).c_str(), (ui->Address->text().toStdWString() + L"\\" + find_txt_name_wstring).c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_COPY_ALLOWED)) { // перемещаяем очередной txt-файл из секретной папки
                            ...
                            //если txt-файлы при востановленни нужно оставлять в секретной папке
                            if (CopyFile((ui->Address_secret->text().toStdWString() + L"\\" + find_txt_name_wstring).c_str(), (ui->Address->text().toStdWString() + L"\\" + find_txt_name_wstring).c_str(), MOVEFILE_REPLACE_EXISTING)) { // перемещаяем очередной txt-файл из секретной папки
    ...
    if(ui->Option_3->isChecked()){ // нужно ли удалить секретную папку если она пуста
        if (!(GetFileAttributes((LPCTSTR)(ui->Address_secret->text().toStdWString()).c_str()) == 0xFFFFFFFF)) { // проверяем существует ли наша секретная папка
            ...
            std::wstring txt_dir_wstring = ui->Address_secret->text().toStdWString(); // адрес секретной папки
            h_txt_file_in_dir = FindFirstFile((LPCTSTR)(txt_dir_wstring + L"\\*.txt").c_str(), &find_txt_file_data); // находим хендел txt-файла из дерриктории
            ...
            if (h_txt_file_in_dir == INVALID_HANDLE_VALUE) { // если секретная папка пуста
                ...
                ui->Log->setPlainText(ui->Log->toPlainText() + "Секретная папка не удалена, т.к. она содержит txt-файл(ы)\n"); // меняем лог
            }
        }
    }
}
